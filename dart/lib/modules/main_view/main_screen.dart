...
import 'package:yuri_reader/modules/history/providers/isar_providers.dart';
import 'package:yuri_reader/repositories/history_repository.dart';
import 'package:yuri_reader/repositories/source_repository.dart';
import 'package:yuri_reader/repositories/update_repository.dart';
import 'package:yuri_reader/modules/more/about/providers/download_file_screen.dart';
import 'package:yuri_reader/modules/more/providers/downloaded_only_state_provider.dart';
...
import 'package:yuri_reader/modules/widgets/loading_icon.dart';
import 'package:yuri_reader/services/fetch_item_sources.dart';
import 'package:yuri_reader/modules/main_view/providers/migration.dart';
...
import 'package:yuri_reader/modules/more/about/providers/check_for_update.dart';
import 'package:yuri_reader/modules/more/data_and_storage/providers/auto_backup.dart';
...
class _MainScreenState extends ConsumerState<MainScreen> {
  ...
  List<NavigationRailDestination> _buildNavigationWidgetsDesktop(
    ...
    // MAL-Sync is the fork's own tab, between Browse and More: the list of
    // everything the other tabs track.
    if (dest.contains("/malsync")) {
      destinations[dest.indexOf("/malsync")] = NavigationRailDestination(
        // Even breathing room between tabs on TV; null off-TV.
        padding: isTv ? const EdgeInsets.symmetric(vertical: 6) : null,
  ...
  List<Widget> _buildNavigationWidgetsMobile(
    ...
    if (dest.contains("/malsync")) {
      destinations[dest.indexOf("/malsync")] = NavigationDestination(
        selectedIcon: const Icon(Icons.sync_alt),
        icon: const Icon(Icons.sync_alt),
        label: "MAL-Sync",
      );
...
class _TabletLayoutState extends State<_TabletLayout> {
  ...
  void _onDestinationTapped(BuildContext context, int newIndex) {
    ...
    // MAL-Sync is not a library tab: its screen opens over whatever is being
    // looked at, and the rail keeps the tab it had selected.
    if (dest == '/malsync') {
      _lastNavTapIndex = null;
      _lastNavTapTime = null;
      Navigator.of(context).push(
        MaterialPageRoute(builder: (_) => const MalSyncScreen()),
      );
      return;
    }
