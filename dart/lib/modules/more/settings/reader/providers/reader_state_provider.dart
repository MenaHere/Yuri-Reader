...
class NavigationOrderState extends _$NavigationOrderState {
  final items = [
    ...
    '/history',
    '/browse',
    // MAL-Sync's own screen. It sits between Browse and More because it is
    // neither a library tab nor a settings page: it is the list of everything
    // tracked, which is what the other tabs feed.
    '/malsync',
    '/more',
    '/trackerLibrary',
  ];
  ...
  List<String> _checkMissingItems(List<String> navigationOrder) {
    // An item added since the order was last saved belongs where the default
    // order puts it, not at the end: appending it would leave it away from the
    // tabs it belongs between. It goes after the last item before it that is
    // already there, and at the front if there is none.
    for (final item in items) {
      if (navigationOrder.contains(item)) continue;
      var at = 0;
      for (var i = items.indexOf(item) - 1; i >= 0; i--) {
        final before = navigationOrder.indexOf(items[i]);
        if (before != -1) {
          at = before + 1;
          break;
        }
      }
      navigationOrder.insert(at, item);
    }
