...
import 'package:yuri_reader/modules/malsync/malsync_arrow.dart';
import 'package:yuri_reader/modules/malsync/malsync_correction.dart';
import 'package:yuri_reader/modules/malsync/malsync_panel.dart';
import 'package:yuri_reader/modules/manga/detail/widgets/manga_cover_viewer.dart';
import 'package:yuri_reader/modules/manga/detail/widgets/readmore.dart';
import 'package:yuri_reader/modules/manga/detail/widgets/split_chapters_dialog.dart';
...
class _MangaInfoHeaderState extends ConsumerState<MangaInfoHeader> {
  ...
  /// malsync's panel, or nothing.
  ///
  /// It belongs to the reading types, not to anime, and it only appears once
  /// malsync is one of the trackers this title's page can write through: a
  /// title nobody tracks has no account to read the panel from. The band it
  /// sits in is the whole width of the column, under the title block and above
  /// the action row.
  Widget _malSyncPanel() {
    ...
    return StreamBuilder(
      ...
      builder: (context, snapshot) {
        ...
        final enabled = entries.any(
          (entry) => entry.syncId == TrackerProviders.malsync.syncId,
        );
        if (!enabled) return const SizedBox.shrink();
        // The hand-picked match is the panel's key: correcting the match
        // rebuilds the panel against the entry that was picked.
        return StreamBuilder(
          stream: trackRepository.watchBySyncIdAndMangaId(
            TrackerProviders.malsync.syncId,
            widget.manga.id!,
          ),
          builder: (context, snapshot) {
            final rows = snapshot.hasData ? snapshot.data! : const <Track>[];
            final matchUrl = rows.isEmpty ? null : rows.first.trackingUrl;
            return MalSyncPanel(
              key: ValueKey(matchUrl),
  ...
  Widget _action() {
    return StreamBuilder(
      ...
      builder: (context, snapshot) {
        ...
        // With malsync switched on it is the tracker this page writes through,
        // so its own mark takes the place of the sync icon, and the button
        // opens the panel that fixes the match rather than the tracker sheet.
        final malsync =
            widget.manga.itemType != ItemType.anime &&
            entries.any(
              (entry) => entry.syncId == TrackerProviders.malsync.syncId,
        ...
        if (malsync) {
          return Expanded(
            child: ElevatedButton(
              ...
              onPressed: () =>
                  showMalSyncCorrection(context, manga: widget.manga),
              child: Column(
                children: [
                  MalSyncArrowIcon(color: context.primaryColor, size: 20),
                  const SizedBox(height: 4),
                  Text(
                    l10nLocalizations(context)!.tracking,
