...
exports.predictionXhrGET = predictionXhrGET;
exports.predictionXhrPOST = predictionXhrPOST;
exports.single = single;
exports.progressIsOld = progressIsOld;
exports.getProgress = getProgress;
exports.getProgressTypeList = getProgressTypeList;
exports.notificationCheck = notificationCheck;
const Cache_1 = require("../utils/Cache");
async function predictionXhrGET(type, apiCacheKey) {
    if (!apiCacheKey)
        return [];
    const response = await api.request.xhr('GET', `https://api.malsync.moe/nc/mal/${type}/${apiCacheKey}/pr`);
    return JSON.parse(response.responseText);
}
async function predictionXhrPOST(type, malDATA) {
    if (malDATA === null)
        return [];
    if (malDATA.length <= 0)
        return [];
    ...
    return returnArray.reduce((acc, val) => acc.concat(val), []);
}
async function single(el, type, mode = 'default', logger = con.m('release')) {
    if (!mode)
        mode = 'default';
    ...
    const releaseItem = await api.storage.get(`release/${type}/${el.cacheKey}`);
    ...
    let finished = false;
    if (progressValue && progressValue.state && progressValue.state === 'complete')
        finished = true;
    ...
    if (config.fallbackPrediction && top && !top.predicition && top.lastEp && top.lastEp.timestamp) {
        ...
        if (predTime && predTemp && predTemp.predicition) {
            if (top.lastEp.total === predTemp.lastEp.total) {
                ...
                if (Math.abs(predTime - (predTemp.predicition.timestamp - 7 * 24 * 60 * 60 * 1000)) <
                    30 * 60 * 60 * 1000) {
                    top.predicition = {
                        timestamp: predTime,
                        probability: 'medium',
...
async function getProgressTypeList(type) {
    const cacheObj = new Cache_1.Cache(`ProgressTypeList${type}`, 24 * 60 * 60 * 1000, false);
...
async function notificationCheck(el, cProgress, nProgress, type) {
    try {
        if ((type === 'anime' && !api.settings.get('progressNotificationsAnime')) ||
...
//# sourceMappingURL=releaseProgressUtils.js.map