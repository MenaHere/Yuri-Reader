...
class MetaOverview extends metaOverviewAbstract_1.MetaOverviewAbstract {
    ...
    statistics(data) {
        ...
        try {
            ...
            j.$.each(j.$(tempHtml).filter('div').slice(0, 5), function (index, value) {
                ...
                const body = typeof j.$(value).find('span[itemprop=ratingValue]').height() !== 'undefined'
                ...
                if (!aTags.length) {
                    body = textTags.map(el => {
                        ...
                        if (match) {
                            ...
                            const weekDate = (0, time_1.getWeektime)(dayString, timeString);
                            if (weekDate) {
                ...
                switch (title.trim()) {
                    ...
                    case 'Aired:': {
                        ...
                        if (range.includes(' to ')) {
                            ...
                            const rangeDate = new IntlWrapper_1.IntlRange(start, end).getDateTimeRangeText();
                            body = [{ text: rangeDate }];
                            break;
                        }
                        // NOTE - For movies/titles with only 1 air date
                        body = [{ text: `${new IntlWrapper_1.IntlDateTime(range).getDateTimeText()}` }];
                        break;
                    }
                    case 'Duration:': {
                        title = api.storage.lang('overview_sidebar_Duration');
                        const durationString = body[0].text;
                        const match = durationString.match(/\d+/g);
                        // NOTE - For anime with episodes
                        if (match) {
                            if (durationString.includes('min') && match.length === 1) {
                                const minutes = Number(match[0]);
                                const result = new IntlWrapper_1.IntlDuration()
                            ...
                            const result = new IntlWrapper_1.IntlDuration().setDuration({ hours, minutes }).getRelativeText();
                            body = [{ text: `${result}` }];
                            break;
                        }
                    }
    ...
    openingSongs(data) {
        const openingSongs = [];
        try {
            const openingBlock = `<table border${data.split('opnening">')[1].split('<table border')[1].split('</table>')[0]}</table>`;
    ...
    endingSongs(data) {
        const endingSongs = [];
        try {
            const endingBlock = `<table border${data.split(' ending">')[1].split('<table border')[1].split('</table>')[0]}</table>`;
    ...
    related(data) {
        ...
        try {
            ...
            j.$.each(j.$(related).filter('.related-entries').find('.entry .content'), function (index, value) {
                const links = [];
                j.$(value)
                    .find('a')
