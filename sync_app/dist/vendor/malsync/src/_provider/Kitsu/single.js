...
const point20decimal_1 = require("../ScoreMode/point20decimal");
const smiley4_1 = require("../ScoreMode/smiley4");
const point10decimal_1 = require("../ScoreMode/point10decimal");
class Single extends singleAbstract_1.SingleAbstract {
    constructor(url) {
        super(url);
        this.url = url;
        this.shortName = 'Kitsu';
        this.authenticationUrl = 'https://kitsu.app/404?mal-sync=authentication';
    ...
    _getScore() {
        if (!this.listI().attributes.ratingHundred)
            return 0;
    ...
    async _update() {
        ...
        if (Number.isNaN(this.ids.kitsu.id)) {
            ...
            try {
                const relation = kitsuRes.data.find(item => item.relationships.item.data.type === this.getType());
                if (!relation)
                    throw new Errors_1.NotFoundError('Not found');
    ...
    apiCall(mode, url, variables = {}, authentication = true) {
        return helper.apiCall(mode, url, variables, authentication).catch(e => {
            if (e instanceof Errors_1.NotAutenticatedError)
                throw new Errors_1.NotAutenticatedError(e.message);
            throw e;
        });
    }
    kitsuSlugtoKitsu(kitsuSlug, type) {
        return this.apiCall('GET', `https://kitsu.app/api/edge/${type}?filter[slug]=${kitsuSlug}&page[limit]=1&include=mappings`, {})
            .catch(e => {
            if (e instanceof Errors_1.NotAutenticatedError) {
                this._authenticated = false;
                return this.apiCall('GET', `https://kitsu.app/api/edge/${type}?filter[slug]=${kitsuSlug}&page[limit]=1&include=mappings`, {}, false);
            }
            throw e;
        })
            .then(res => {
    ...
    malToKitsu(malid, type) {
        return this.apiCall('GET', `https://kitsu.app/api/edge/mappings?filter[externalSite]=myanimelist/${type}&filter[externalId]=${malid}&include=item&fields[item]=id`, {})
            .catch(e => {
            if (e instanceof Errors_1.NotAutenticatedError) {
                this._authenticated = false;
                return this.apiCall('GET', `https://kitsu.app/api/edge/mappings?filter[externalSite]=myanimelist/${type}&filter[externalId]=${malid}&include=item&fields[item]=id`, {}, false);
            }
            throw e;
        })
            .then(res => {
    ...
    getScoreMode() {
        switch (api.settings.get('kitsuOptions').ratingSystem) {
            case 'simple':
                return smiley4_1.smiley4;
            case 'regular':
                return point10decimal_1.point10decimal;
            case 'advanced':
                return point20decimal_1.point20decimal;
            default:
