...
class Single extends singleAbstract_1.SingleAbstract {
    constructor(url) {
        ...
        this.displayUrl = '';
        this.shortName = 'MangaBaka';
        this.authenticationUrl = helper_1.authenticationUrl;
        this.forceSeries = null;
        this.forceLibraryEntry = null;
        this.logger = helper_1.logger;
        return this;
    }
    handleUrl(url) {
    ...
    _getStatus() {
        return (0, helper_1.bakaStateToState)(this.libraryEntry.state);
    }
    _setStatus(status) {
        this.libraryEntry.state = (0, helper_1.stateToBakaState)(status);
    }
    _getStartDate() {
        if (!this.libraryEntry.start_date)
            return null;
        return (0, helper_1.timestampToDate)(this.libraryEntry.start_date);
    }
    _setStartDate(startDate) {
        ...
        this.libraryEntry.start_date = (0, helper_1.dateToTimestamp)(startDate);
    }
    _getFinishDate() {
        if (!this.libraryEntry.finish_date)
            return null;
        return (0, helper_1.timestampToDate)(this.libraryEntry.finish_date);
    }
    _setFinishDate(finishDate) {
        ...
        this.libraryEntry.finish_date = (0, helper_1.dateToTimestamp)(finishDate);
    }
    _getRewatchCount() {
    ...
    _getScore() {
        if (Number(this.libraryEntry.rating) === 0)
            return 0;
    ...
    setStreamingUrl(streamingUrl) {
        if (streamingUrl) {
            ...
            if (isValid)
                this.libraryEntry.read_link = streamingUrl;
        }
    ...
    _getImage() {
        return (0, helper_1.getImageUrl)(this.libraryEntry.Series, 'large');
    }
    _getRating() {
    ...
    async _update() {
        ...
        if (this.forceSeries) {
            ...
            seriesEntry = await (0, seriesService_1.getSeries)(this.ids);
        }
        if (!seriesEntry) {
        ...
        if (this.forceLibraryEntry || this.forceSeries) {
            ...
            try {
                json = (await (0, helper_1.call)(helper_1.urls.libraryEntry(seriesEntry.id))).data;
            }
            catch (e) {
    ...
    async _sync() {
        ...
        return (0, helper_1.call)(helper_1.urls.libraryEntry(this.ids.baka), entryToSend, method);
    }
    getScoreMode() {
        return point100_1.point100;
    }
    _delete() {
        return (0, helper_1.call)(helper_1.urls.libraryEntry(this.ids.baka), {}, 'DELETE');
    }
}
